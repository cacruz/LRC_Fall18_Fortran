...
\begin{itemize}
\item Every extended type has an implicit component associated with the parent type
  \begin{itemize}
  \item The component name is the type name of the parent.
  \item Provides multiple mechanisms to access components in parent type
  \end{itemize}
\end{itemize}
...
\item An extended type can \emph{override} a type-bound procedure in the parent - specifying new behavior in the extended type.
  ...
  \quad procedure, \textcolor{blue}{NON\_OVERRIDABLE} :: foo
  \end{itemize}
\end{itemize}
...
...
type, extends(AbstractShape) :: Square   real :: lengthcontains   procedure :: area => squareArea !  Provide concreteend type Square
\end{lstlisting}
}
