...
Members of the Advanced Software Technology Group (ASTG) Code 606, NASA GSFC:
\begin{itemize}
...
\begin{center}
\textcolor{blue}{Introduction to Fortran 90}\\
...
\begin{itemize}
	\item Use a good editor, e.g. vim or emacs 
\end{itemize}
% so, use a good editor capable of syntax highlighting
\item Fortran source code is \textbf{case insensitive}, i.e. \emph{X} is the same \emph{x}
\item Avoid using  mixed case for statements and variables. It is not considered a \textbf{best practice}
\item Use uppercase letters for Fortran keywords ... or not
\begin{itemize}
	\item Use one convention and be consistent. 
\end{itemize}
\end{itemize}
...
The general structure of a Fortran \textbf{program} is as follows:
\FortranStyle

 \begin{lstlisting}[style=Fortran]
PROGRAM program_name ! keyword PROGRAM
IMPLICIT NONE   !  i.e. all variable MUST be declared
...
END PROGRAM
\end{lstlisting}
The unit containing the PROGRAM attribute is often called the \textbf{main program} or \textbf{main}.
\end{frame}
